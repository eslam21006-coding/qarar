...
Part B's **root-cause fix (US2 → the change that re-keys settings to
the right account or identity)** is the only thing FR-015 gates.
Adding a retention/TTL mechanism would solve a problem that does not
exist and is explicitly out of scope. The Part B **safeguards** in
US3 (failed-load protection) and US4 (overwrite prevention) are NOT
gated by FR-015: they fix user-visible harm that exists independently
of the root cause, and they can ship in parallel with the diagnosis
because they cannot mask or invalidate whatever US2 finds. They
MUST still preserve the data-preservation guarantee in FR-024.
...
**Independent Test**: Two separate, deterministic tests:

  1. **Snapshot equivalence** — replay captured Meta API responses
     (both success and failure cases) through the snapshot builder
     twice: once via the new concurrent path, once via the old
     sequential path, using the same captured responses both times.
     Every field of the resulting snapshot must be identical. Live
     calls are NOT used here, because the wall clock, the rate-limit
     state, and even the underlying data can change between runs, and
     any of those would invalidate the equivalence check.

  2. **Latency and rate-limit behaviour** — run a separate,
     controlled test that times the concurrent path against the
     sequential path on the same inputs and asserts the wall-clock
     reduction (SC-001). Rate-limit observations from this run are
     recorded for FR-007, not derived from the equivalence test.
...
- **FR-002**: The resulting snapshot MUST be identical to what the sequential implementation produced for the same inputs. This is a timing-only change; no value, rounding, ordering, or field may differ. Equivalence MUST be verified by replaying captured Meta API responses (success and failure cases) through both paths, not by comparing two live runs.
...
- **FR-015**: Only the confirmed root cause MAY be fixed. Speculative fixes for unconfirmed theories MUST NOT be shipped in this feature. FR-015 gates the **root-cause fix** for Part B (the change that re-keys settings to the right account or identity). It does NOT gate the Part B safeguards in FR-017 through FR-022, which fix user-visible harm that exists independently of which root cause is eventually confirmed.
...
- **SC-003**: Snapshots produced after the change are identical to those produced before it for the same captured inputs — verified by replaying the same captured Meta API responses (success and failure cases) through both paths, with zero differing fields.
