...
**Crash-window tradeoff (resolved)**: between the delete and the insert, a crash would have left the user with zero Pages, hiding the section until their next sync. With `db.transaction(async tx => ...)` wrapping both statements (see "Transactional storage guarantee" below), a crash **before** the transaction commits rolls back the entire replacement — no partial state, no orphaned rows. A crash **after** commit lands the new set atomically. The window between the statements is sub-second, and writes only begin after a successful fetch — the damage is at most a hidden confirmation strip, not lost user data. Wrap in a transaction if the driver path makes it free; do not add a transaction abstraction solely for this.

**Transactional storage guarantee**: `syncPages` wraps the delete + insert in `db.transaction(async tx => ...)` (`server/db.ts:381-394`). The `facebookPages` migration (`drizzle/0011_facebook_pages.sql`) does not name a storage engine, so we depend on the deployment default. **The deployment MUST use a transactional engine (InnoDB on MySQL, or any engine on TiDB) — the unique `(userId, pageId)` index and `db.transaction` rely on it.** Without a transactional engine, a crash between the delete and the insert commits half the sync (destructive). A crash before `db.transaction` commits rolls back the entire replacement (no partial state); a crash after commit commits the new set atomically. The deployment guarantee is documented in `quickstart.md` as part of the migration prerequisites.
