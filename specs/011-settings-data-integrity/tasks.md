...
- [ ] T037 [US4] **Gated on T034.** Apply the unique index **by hand**: `npx tsx scripts/apply-migrations.mjs drizzle/0010_settings_unique_index.sql`. **DO NOT add `uniqueIndex(...)` to `drizzle/schema.ts` — doing so defeats the gate.** The constraint lives *only* in `drizzle/0010_settings_unique_index.sql`, which is deliberately **absent from `drizzle/meta/_journal.json`**, so `drizzle-kit migrate` (which iterates only over `journal.entries`) can never apply it automatically. The moment the index is added to `schema.ts`, the next `drizzle-kit generate` emits it *into* the journal and the following deploy applies it ungated — before the diagnose → repair → verify-clean cycle has run, against a table that may still hold duplicates. That is the exact failure this gate exists to prevent. See the header comment in `drizzle/0010_settings_unique_index.sql` and `drizzle/schema.ts:100-111`. Hand-check the SQL for TiDB before applying
